...
# Generating ASV abundance and Taxonomy data (Data S2)

```{r}
# Extract abundance table as a data.frame
...
# Pivot  abundance table to long format and join taxonomy
integrated_df <- abund_df %>%
  pivot_longer(
    cols = -SampleID,
    names_to  = "ASV",
    values_to = "Abundance"
...
run_dbrda <- function(ps,
                      ...
                      trans = "compositional", # modify as pa for presence/absence or compositional for abundance
                      binary = TRUE,
                      otusqrt = TRUE,
                      distsqrt = FALSE,
                      permanova = TRUE,
...
sig.tab.bayes.p.fixed.hemo <- sig.tab.bayes %>%
  ...
  theme(axis.text.y = element_text(size = 7, margin = margin(r = 5)))
